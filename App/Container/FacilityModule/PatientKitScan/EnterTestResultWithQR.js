...
class EnterTestResultWithQR extends ValidationComponent {
  constructor(props) {
    super(props);
    this.state = {
      IsMessageShow: false,
  ...
  async _ConfirmPress() {
    ...
    const payload = {};
    payload.PatientId = this.props.currentPatient.PatientId;
    payload.KitNo = this.props.currentPatient.KitNo;
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={[Helpers.bottomView, { position: 'relative', backgroundColor: 'white' }]}>
          <View style={[Helpers.btnContainer, { marginBottom: 15, marginTop: 4 }]}>
            <LinearGradient
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
...
EnterTestResultWithQR.propTypes = {
